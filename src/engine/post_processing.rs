...
use crate::engine::minigames::ActiveMinigame;
...
pub(super) fn plugin(app: &mut App) {
    ...
    if let Some(subgraph) = graph.get_sub_graph(Core2d) {
...
impl ViewNode for PostProcessNode {
    ...
    type ViewQuery = (
        &'static ViewTarget,
        // This makes sure the node only runs on cameras with the PostProcessingSettins component
    ...
    // If you don't want your effect to run on ever camera,
...
fn update_settings(
    ...
    for mut s in &mut settings {
        ...
        // Smooth lerp so it fades in/out rather than snapping
        s.intensity += (target_intensity - s.intensity) * (dt * 4.0).min(1.0);
    }
}
