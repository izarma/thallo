...
use crate::engine::{asset_tracking::ResourceHandles, screens::Screen};
...
pub(super) fn plugin(app: &mut App) {
    ...
    app.add_systems(
        Update,
        enter_desktop_screen.run_if(in_state(Screen::Loading).and(is_startup_done)),
    );
}
...
fn spawn_startup(mut commands: Commands, asset_server: Res<AssetServer>) {
    commands
        .spawn((
            StreamingAnimatedImageController::play(asset_server.load("ui/startup.gif")),
...
fn enter_desktop_screen(mut next_screen: ResMut<NextState<Screen>>) {
